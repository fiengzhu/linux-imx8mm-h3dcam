		...
		uint32_t size, uint32_t count, const char *buf)
{
	...
	dev_dbg(&client->dev, "i2c attempt write reg=0x%x\n", reg);
...
static bool avt_check_fmt_available(struct i2c_client *client, u32 media_bus_fmt)
{
	...
	dev_info(&client->dev, "avt h3d available pixel formats (%llx)\n", avail_mipi);
...
#endif
		/* reset dphy once, othewise it can cause frame corrupt, but is necessary to  */
		//if (!priv->phyres_done) // comment it, otherwise imx8 can't receive image
		{
			avt_set_param(priv->client, V4L2_AV_CSI2_PHY_RESET, 1); // disable dphy first
			msleep(10);
			avt_set_param(priv->client, V4L2_AV_CSI2_PHY_RESET, 0); // enable dphy
			priv->phyres_done = true;
		}
		...
		struct v4l2_subdev_format *format)
{
	...
	//avt_info(sd, "h3d:avt_csi2_get_fmt getting format.\n");
		...
		struct v4l2_subdev_format *format)
{
		 ...
		 format->format.height != priv->frmp.r.height))
	{
		...
		if (priv->mode == AVT_BCRM_MODE) avt_err(sd, "|-- cam mode in AVT_BCRM_MODE\n");
		else avt_err(sd, "|-- cam mode in AVT_GENCP_MODE\n");
		avt_err(sd, "|-- Request (%d, %d), cam (%d, %d)\n", format->format.width, format->format.height, priv->frmp.r.width, priv->frmp.r.height);
		return -EINVAL;
	}
	...
	if(ret < 0)
	{
		avt_err(sd, "pixel format setting error with %x.\n", format->format.code);
		return ret;
	}
	else 
	{
		avt_info(sd, "pixel format set to %x.\n", format->format.code);
	}
	...
	for(i = 0; i < ARRAY_SIZE(avt_mbus_formats); i++){
		if(avt_check_fmt_available(client, avt_mbus_formats[i])
		   && !avt_mbus_fmt_is_hidden(avt_mbus_formats[i])) {
			...
			avt_info(sd, "--> available mbux format (%d), %x", i, avt_mbus_formats[i]);
		}
	}
				...
				struct v4l2_subdev_mbus_code_enum *code)
{
	...
	avt_info(sd, "avt_csi2_enum_mbus_code, %d: %x\n", code->index, code->code);
	if (code->code == -EINVAL)
		return -EINVAL;
				...
				struct v4l2_subdev_frame_size_enum *fse)
{
	...
	//int i;
	int ret;
	unsigned int width_min, width_max, height_min, height_max;
	...
	ret = avt_reg_read(client,
			...
			(char *) &width_min);
	if (ret < 0) { dev_err(&client->dev, "read IMG_WIDTH_MIN_32R failed\n");
		return ret;}
	...
	ret = avt_reg_read(client,
			...
			AV_CAM_REG_SIZE, AV_CAM_DATA_SIZE_32,
			(char *) &width_max);
	if (ret < 0) { dev_err(&client->dev, "read IMG_WIDTH_MAX_32R failed\n");
		return ret;}
	...
	ret = avt_reg_read(client,
			...
			AV_CAM_REG_SIZE, AV_CAM_DATA_SIZE_32,
			(char *) &height_min);
	if (ret < 0) {dev_err(&client->dev, "read IMG_HEIGHT_MIN_32R failed\n");
		return ret;}
	...
	ret = avt_reg_read(client,
			...
			AV_CAM_REG_SIZE, AV_CAM_DATA_SIZE_32,
			(char *) &height_max);	
	if (ret < 0) { dev_err(&client->dev, "read IMG_HEIGHT_MAX_32R failed\n");
		return ret;}	

	fse->min_width = width_min;
	fse->max_width = width_max;
	fse->min_height = height_min;
	fse->max_height = height_max;
		...
		struct v4l2_queryctrl *qctrl)
{
	...
	case V4L2_CID_BRIGHTNESS:
		...
		avt_info(sd, "control OK 'Brightness' supported by firmware\n");
		break;
	...
	case V4L2_CID_EXPOSURE:
		...
		avt_info(sd, "control OK 'Exposure' supported by firmware\n");
		break;
	...
	case V4L2_CID_EXPOSURE_ABSOLUTE:
		...
		avt_info(sd, "control OK 'Exposure Absolute' supported by firmware\n");
		break;
	...
	case V4L2_CID_EXPOSURE_AUTO:
		...
		avt_info(sd, "control OK 'Exposure Auto' supported by firmware\n");
		break;
	...
	case V4L2_CID_GAIN:
		...
		avt_info(sd, "control OK 'Gain' supported by firmware\n");
		break;
	...
	case V4L2_CID_AUTOGAIN:
		...
		avt_info(sd, "control OK 'Auto Gain' supported by firmware\n");
		break;
	...
	case V4L2_CID_HFLIP:
		...
		avt_info(sd, "control OK 'Reversing X (Horizantal Flip)' supported by firmware\n");
		break;
	...
	case V4L2_CID_VFLIP:
		...
		avt_info(sd, "control OK 'Reversing Y (Vertical Flip)' supported by firmware\n");
		break;
	...
	case V4L2_CID_GAMMA:
		...
		avt_info(sd, "control OK 'Gamma' supported by firmware\n");
		break;
	...
	case V4L2_CID_CONTRAST:
		...
		avt_info(sd, "control OK 'Contrast' supported by firmware\n");
		break;
	...
	case V4L2_CID_AUTO_WHITE_BALANCE:
		...
		avt_info(sd, "control OK 'White Balance Auto' supported by firmware\n");
		break;
	...
	case V4L2_CID_DO_WHITE_BALANCE:
		...
		avt_info(sd, "control OK 'White Balance' supported by firmware\n");
		break;
	...
	case V4L2_CID_SATURATION:
		...
		avt_info(sd, "control OK 'Saturation' supported by firmware\n");
		break;
	...
	case V4L2_CID_HUE:
		...
		avt_info(sd, "control OK 'Hue' supported by firmware\n");
		break;
	...
	case V4L2_CID_RED_BALANCE:
		...
		avt_info(sd, "control OK 'Red Balance' supported by firmware\n");
		break;
	...
	case V4L2_CID_BLUE_BALANCE:
		...
		avt_info(sd, "control OK 'Blue Balance' supported by firmware\n");
		break;
	...
	case V4L2_CID_SHARPNESS:
		...
		avt_info(sd, "control OK 'Sharpness' supported by firmware\n");
		break;
...
static int avt_s_parm(struct v4l2_subdev *sd, struct v4l2_streamparm *parm)
{
	...
	//avt_info(sd, "h3d, numerator req %d, set %d\n", parm->parm.capture.timeperframe.numerator, tpf->numerator);
	//avt_info(sd, "h3d, denominator req %d, set %d\n", parm->parm.capture.timeperframe.denominator, tpf->denominator);
	...
	/* Setting '0' to the Frame Start Trigger mode: Disable triggering */
	vc.value = 0;
	ret = ioctl_bcrm_i2cwrite_reg(client, &vc, priv->cci_reg.bcrm_addr + FRAME_START_TRIGGER_MODE_8RW, AV_CAM_DATA_SIZE_8);
	if (ret < 0) {
		avt_err(sd, "ACQUISITION_FRAME_START_TRIGGER_MODE_8RW: i2c write failed (%d)\n",
				ret);
		return ret;
	}
	...
	avt_info(sd, "Camera capability is %lld to %lld (%lld), ACQUISITION_FRAME_RATE set to: %lld(%lld.%lldHz, manual=%d)\n", 
	min, max, step, value64, value64/UHZ_TO_HZ, value64%UHZ_TO_HZ/FRAQ_NUM, manual_en);	
...
/* Check the checksum of received with calculated. */
	...
	dev_info(&client->dev, "cci bcrm_addr: 0x%x\n",
			priv->cci_reg.bcrm_addr);
	dev_info(&client->dev, "cci checksum: 0x%x\n",
			priv->cci_reg.checksum);

	dev_info(&client->dev, "cci manufacture_name: %s\n", 
...
/* Write image format from camera,
 * should be only called once, during initialization
 * */
static int avt_write_fmt_to_device(struct v4l2_subdev *sd, uint32_t fmt)
{
	struct avt_csi2_priv *priv = avt_get_priv(sd);
	struct i2c_client *client = priv->client;
	uint32_t avt_img_fmt = fmt;
	uint8_t bayer_temp = 0;
	int ret = 0;

	switch (avt_img_fmt) {
		case MEDIA_BUS_FMT_RGB444_1X12:
			avt_img_fmt = MIPI_DT_RGB444;
			break;
		case MEDIA_BUS_FMT_RGB565_1X16:
			avt_img_fmt = MIPI_DT_RGB565;
			break;
		case MEDIA_BUS_FMT_RGB888_1X24:
		case MEDIA_BUS_FMT_BGR888_1X24:
		case MEDIA_BUS_FMT_RGB888_1X32_PADHI:
			avt_img_fmt = MIPI_DT_RGB888;
			break;
		case MEDIA_BUS_FMT_VYUY8_2X8:
			avt_img_fmt = MIPI_DT_YUV422;
			break;
		/* RAW 8 */
		case MEDIA_BUS_FMT_Y8_1X8:
			avt_img_fmt = MIPI_DT_RAW8;
			bayer_temp = monochrome;
			break;
		case MEDIA_BUS_FMT_SBGGR8_1X8:
			avt_img_fmt = MIPI_DT_RAW8;
	...
	if (ret < 0) {
		dev_err(&client->dev, "i2c write format failed (%d)\n", ret);
		return ret;
	}
...
static int avt_init_mode(struct v4l2_subdev *sd)
{
	...
	avt_info(sd, " -> supported lanes: 4(%d), 3(%d), 2(%d), 1(%d)\n", 
		test_bit(3, (const long *)(&avt_supported_lane_counts)),
		test_bit(2, (const long *)(&avt_supported_lane_counts)),
		test_bit(1, (const long *)(&avt_supported_lane_counts)),
		test_bit(0, (const long *)(&avt_supported_lane_counts)));
	...
	avt_info(sd, "csi clock camera range: %d:%d Hz, host range: %d:%d Hz\n",
		avt_min_clk, avt_max_clk,
		CSI_HOST_CLK_MIN_FREQ, CSI_HOST_CLK_MAX_FREQ);
	avt_info(sd, "request freq: %d Hz\n", priv->csi_clk_freq);
	...
	avt_info(sd, "csi clock read from camera: %d Hz\n", avt_max_clk);

	/* set camera format */
	//added by yuefeng
	ret = avt_write_fmt_to_device(sd, MIPI_DT_RGB888);
	if (ret < 0)
		avt_err(sd, "camera format writing error\n");
	// end

	ret = avt_read_fmt_from_device(sd, &(priv->mbus_fmt_code));
	if (ret < 0)
		return ret;
	avt_info(sd, "camera format set to: %x\n", priv->mbus_fmt_code);
	...
	/* set exposure continously auto */
	ret = avt_reg_write(priv->client,
			priv->cci_reg.bcrm_addr + EXPOSURE_AUTO_8RW,
			2);

	/* set white balance once auto */
	ret = avt_reg_write(priv->client,
			priv->cci_reg.bcrm_addr + WHITE_BALANCE_AUTO_8RW,
			1);
	...
	/* mark phy reset status */
	priv->phyres_done = false;
	return 0;
}
			...
			const struct i2c_device_id *id)
{
	...
	endpoint = v4l2_fwnode_endpoint_alloc_parse(ep);
	if (IS_ERR(endpoint)) {
	...
	dev_info(&client->dev, "Frame rate read back is %lld:\n", framerate);
...
//TODO: GENCP
    ...
    dev_info(dev, " -> %s: sysfs group created! (%d)\n", __func__, ret);
	if (ret) {
